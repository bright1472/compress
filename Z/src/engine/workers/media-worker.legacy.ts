...
async function runPipeline({ file, config, outputHandle }: any) {
  ...
  const actualDurationSeconds = sampleBuffer.reduce((sum, s) => sum + s.duration / s.timescale, 0);
  console.log(`🔍 [Worker] Actual duration: ${actualDurationSeconds.toFixed(2)}s, frames: ${totalFrames}`);
  ...
  const calculatedBitrate = calculateSmartBitrate(width, height, totalSize, actualDurationSeconds, config.bitrate);
...
function calculateSmartBitrate(
  ...
  const originalBitrate = (fileSize * 8) / durationSeconds; // bps
  ...
  if (userBitrate) {
    ...
    if (userBitrate > originalBitrate) {
      console.warn(`⚠️ [Worker] User bitrate (${(userBitrate / 1000000).toFixed(2)}Mbps) > original (${(originalBitrate / 1000000).toFixed(2)}Mbps), using smart bitrate to prevent inflation`);
